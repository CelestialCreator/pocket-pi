...
# Install one-at-a-time with `|| true` so a single failure (e.g. a package
# with a broken native build, like pi-agent-dashboard's node-pty on
# android-arm64) doesn't abort the whole run. Anything that fails just won't
# be registered as a Pi extension in step 3a.
